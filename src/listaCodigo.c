...
struct ListaCRep {	//Definicion de la estructura lista de Código
...
ListaC creaLC() {	//Crear una nueva lista vacia
...
void liberaLC(ListaC codigo) {	//Liberar una lista (Vacía y no vacia)
...
void insertaLC(ListaC codigo, PosicionListaC p, Operacion o) {	//Insertar nuevo elemento a la lista
...
Operacion recuperaLC(ListaC codigo, PosicionListaC p) {	//Recupera un elemento
  assert(p != codigo->ultimo);
  return p->sig->dato;
}

PosicionListaC buscaLC(ListaC codigo, PosicionListaC p, char *clave, Campo campo) { //Buscar un elemento, recibe como parametro la lista a analizar, una posicion dentro de la lista, un campo y la clave
  NodoPtr aux = p;
  char *info;
  while (aux->sig != NULL) {
    switch (campo) {
...
PosicionListaC inicioLC(ListaC codigo) {	//Devuelve el inicio de la lista
  return codigo->cabecera;
}

PosicionListaC finalLC(ListaC codigo) {	//Devuelve el fin de la lista
...
char * recuperaResLC(ListaC codigo) {	// Recupera el registro resultado de una lista de codigo 
  return codigo->res;
}
